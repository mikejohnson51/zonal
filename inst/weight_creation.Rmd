...
knitr::opts_chunk$set(
  out.width = "100%",
  fig.width = 7, 
  fig.height = 4, dev = "CairoPNG", dpi = 150, fig.path = "w-",
...
```{r data-grid}
(meta = raster::stack(file))
```


The grid we are working with holds daily PET values for CONUS for the year 1979. It has:

- `r prettyNum(ncol(meta), big.mark = ",")` X_cells
- `r prettyNum(nrow(meta), big.mark = ",")` Y_cells 
- `r prettyNum(raster::nlayers(meta), big.mark = ",")` time slices 

for a total of `r prettyNum(raster::ncell(meta), big.mark = ",")` values.

```{r data-geom}
glimpse(geom)
```
  
In total we have `r prettyNum(nrow(geom), big.mark = ",")` aggregation units to summarize over the `r prettyNum(raster::nlayers(meta)` time steps.
...
In total we have `r prettyNum(nrow(colorado), big.mark = ",")` aggregation units to summarize over the `r filter(meta, name == 'day')$length` time steps.
